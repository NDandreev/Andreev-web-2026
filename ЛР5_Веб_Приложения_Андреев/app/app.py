...
@app.before_request
def log_visit():
    """Автоматическое логирование всех посещений страниц"""
...
# СТАТИСТИКА И ЖУРНАЛ ПОСЕЩЕНИЙ
@app.route('/statistics')
@login_required
def statistics_journal():
    """Журнал посещений"""
    ...
    for log in pagination.items:
        if log.user_id:
            ...
            log.user_name = user.full_name if user else f"Пользователь {log.user_id}"
        else:
...
@app.route('/statistics/pages')
@login_required
def statistics_pages():
    ...
    return render_template('statistics_pages.html', title='Статистика по страницам', stats=stats)


@app.route('/statistics/users')
@login_required
def statistics_users():
    """Статистика по пользователям (только для админа)"""
    from sqlalchemy import func, desc
    ...
    if role_name != 'admin':
        flash('У вас недостаточно прав для доступа к данной странице.', 'danger')
        return redirect(url_for('statistics_journal'))
    ...
    for idx, stat in enumerate(stats, 1):
        user = User.query.get(stat.user_id) if stat.user_id else None
        result.append({
            'num': idx,
    ...
    return render_template('statistics_users.html', title='Статистика по пользователям', stats=result)


@app.route('/statistics/export/pages')
@login_required
def statistics_export_pages():
...
@app.route('/statistics/export/users')
@login_required
def statistics_export_users():
    """Экспорт статистики по пользователям в CSV (только для админа)"""
    from sqlalchemy import func, desc
    import csv
    from io import StringIO
