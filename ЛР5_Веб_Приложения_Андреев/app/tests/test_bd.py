...
# ========== ТЕСТЫ СТАТИСТИКИ И ЖУРНАЛА ПОСЕЩЕНИЙ (лаб 5)==========

# Тест 28 - Доступ к журналу посещений для администратора
def test_statistics_journal_access_for_admin(admin_client, init_database):
...
# Тест 29 - Доступ к журналу посещений для обычного пользователя
def test_statistics_journal_access_for_user(authenticated_client, init_database):
...
# Тест 30 - Доступ к журналу посещений для гостя запрещён
def test_statistics_journal_access_for_guest(client, init_database):
    response = client.get('/statistics', follow_redirects=True)
    response_text = response.data.decode('utf-8')
    assert 'Для доступа необходимо авторизоваться' in response_text or 'Вход' in response_text


# Тест 31 - Администратор видит все записи в журнале
def test_statistics_journal_admin_sees_all_logs(admin_client, init_database):
    ...
    response = admin_client.get('/statistics')
    response_text = response.data.decode('utf-8')
    assert '/users' in response_text or '/about' in response_text


# Тест 32 - Доступ к статистике по страницам для администратора
def test_statistics_pages_access_for_admin(admin_client, init_database):
    response = admin_client.get('/statistics/pages')
    assert response.status_code == 200
    response_text = response.data.decode('utf-8')
    assert 'Статистика по страницам' in response_text


# Тест 33 - Доступ к статистике по страницам для обычного пользователя
def test_statistics_pages_access_for_user(authenticated_client, init_database):
    response = authenticated_client.get('/statistics/pages')
    assert response.status_code == 200
    response_text = response.data.decode('utf-8')
    assert 'Статистика по страницам' in response_text


# Тест 34 - Доступ к статистике по пользователям только для администратора
def test_statistics_users_access_for_admin(admin_client, init_database):
    response = admin_client.get('/statistics/users')
    assert response.status_code == 200
    response_text = response.data.decode('utf-8')
    assert 'Статистика по пользователям' in response_text


# Тест 35 - Обычный пользователь не имеет доступа к статистике по пользователям
def test_statistics_users_access_denied_for_user(authenticated_client, init_database):
    response = authenticated_client.get('/statistics/users', follow_redirects=True)
    response_text = response.data.decode('utf-8')
    assert 'недостаточно прав' in response_text or 'Журнал посещений' in response_text


# Тест 36 - Экспорт CSV по страницам для администратора
def test_statistics_export_pages_for_admin(admin_client, init_database):
    response = admin_client.get('/statistics/export/pages')
    assert response.status_code == 200
    assert 'text/csv' in response.headers.get('Content-Type', '')
    assert 'attachment' in response.headers.get('Content-Disposition', '')


# Тест 37 - Экспорт CSV по страницам для обычного пользователя
def test_statistics_export_pages_for_user(authenticated_client, init_database):
    response = authenticated_client.get('/statistics/export/pages')
    assert response.status_code == 200
    assert 'text/csv' in response.headers.get('Content-Type', '')


# Тест 38 - Экспорт CSV по пользователям только для администратора
def test_statistics_export_users_for_admin(admin_client, init_database):
    response = admin_client.get('/statistics/export/users')
    assert response.status_code == 200
    assert 'text/csv' in response.headers.get('Content-Type', '')
    assert 'attachment' in response.headers.get('Content-Disposition', '')


# Тест 39 - Обычный пользователь не может экспортировать CSV по пользователям
def test_statistics_export_users_denied_for_user(authenticated_client, init_database):
    response = authenticated_client.get('/statistics/export/users', follow_redirects=True)
    assert response.status_code == 200
    response_text = response.data.decode('utf-8')
    assert 'Статистика по пользователям' not in response_text


# Тест 40 - Пагинация в журнале посещений работает
def test_statistics_journal_pagination(admin_client, init_database):
    # Делаем много запросов для создания записей
    for i in range(25):
        admin_client.get('/users')

    response_page_1 = admin_client.get('/statistics?page=1')
    response_page_2 = admin_client.get('/statistics?page=2')

    assert response_page_1.status_code == 200
    assert response_page_2.status_code == 200

    # Проверяем наличие пагинации
    response_text = response_page_1.data.decode('utf-8')
    assert 'page=2' in response_text or 'Следующая' in response_text


# Тест 41 - Логирование посещений работает
def test_visit_logging_works(client, init_database):
    with client:
        # Создаём пользователя
        login = 'test_log_user'
        created_test_users.append(login)
        ...
        if user is None:
            user = User(
                login=login,
                password_hash=generate_password_hash('LogPass123!'),
                first_name='Лог',
                last_name='Тестовый'
            )
            db.session.add(user)
        ...
        # Авторизуемся и посещаем страницы
        client.post('/login', data={
            'username': login,
            'password': 'LogPass123!'
        }, follow_redirects=True)
